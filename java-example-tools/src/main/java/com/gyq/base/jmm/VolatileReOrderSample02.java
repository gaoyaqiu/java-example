...
/**
 * 并发场景下存在指令重排（通过volatile优化 ）
 *
 *volatile 底层是基于内存屏障实现，主要有四种插入屏障策略：
 *StoreStore屏障: 在每一个volatile写操作的前面插入一个StoreStore屏障
 *StoreLoad屏障：在每一个volatile写操作的后面插入一个StoreLoad屏障
 *LoadLoad屏障：在每一个volatile读操作的后面插入一个LoadLoad屏障
 *LoadStore屏障：在每一个volatile读操作的后面插入一个LoadLoad屏障
 *
 * @author qiu
 * @date 2020/5/3
 */
public class VolatileReOrderSample02 {
