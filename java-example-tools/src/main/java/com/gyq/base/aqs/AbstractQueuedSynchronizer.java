...
public abstract class AbstractQueuedSynchronizer
  ...
  /**
   * Creates a new {@code AbstractQueuedSynchronizer} instance with initial synchronization state of
   * zero.
   */
   ...
   * information about a thread in the predecessor of its node.  A "status" field in each node keeps
   * track of whether a thread should block.  A node is signalled when its predecessor releases.
   * Each node of the queue otherwise serves as a specific-notification-style monitor holding a
   * single waiting thread. The status field does NOT control whether threads are granted locks etc
   * though.  A thread may try to acquire if it is first in the queue. But being first does not
   * guarantee success; it only gives the right to contend.  So the currently released contender
   * thread may need to rewait.
   *
   ...
   * Similarly, dequeuing involves only updating the "head". However, it takes a bit more work for
   * nodes to determine who their successors are, in part to deal with possible cancellation due to
   ...
   * wake up by traversing next link to determine which thread it is.  Determination of successor
   * must avoid races with newly queued nodes to set the "next" fields of their predecessors.  This
   * is solved when necessary by checking backwards from the atomically updated "tail" when a node's
   * successor appears to be null. (Or, said differently, the next-links are an optimization so that
   ...
   * cancellation, allowing them to stabilize on a new predecessor, unless we can identify an
   * uncancelled predecessor who will carry this responsibility.
   *
  ...
  /**
   * Sets the value of synchronization state. This operation has memory semantics of a {@code
   * volatile} write.
   *
  ...
  /**
   * 把当前结点设置为SIGNAL或者PROPAGATE 唤醒head.next(B节点)，B节点唤醒后可以竞争锁，成功后head->B，然后又会唤醒B.next，一直重复直到共享节点都唤醒
   * head节点状态为SIGNAL，重置head.waitStatus->0，唤醒head节点线程，唤醒后线程去竞争共享锁 head节点状态为0，将head.waitStatus->Node.PROPAGATE传播状态，表示需要将状态向后继节点传播
   */
  ...
  /**
   * 阻塞当前节点，返回当前Thread的中断状态 LockSupport.park 底层实现逻辑调用系统内核功能 pthread_mutex_lock 阻塞线程
   */
  ...
  /**
   * 共享式：共享式地获取同步状态。对于独占式同步组件来讲，同一时刻只有一个线程能获取到同步状态， 其他线程都得去排队等待，其待重写的尝试获取同步状态的方法tryAcquire返回值为boolean，这很容易理解；
   * 对于共享式同步组件来讲，同一时刻可以有多个线程同时获取到同步状态，这也是“共享”的意义所在。 本方法待被之类覆盖实现具体逻辑 1.当返回值大于0时，表示获取同步状态成功，同时还有剩余同步状态可供其他线程获取；
   * <p>
   ...
   * acquire in shared mode (that is, this method is invoked from {@link #tryAcquireShared}) then it
   * is guaranteed that the current thread is not the first queued thread.  Used only as a heuristic
   ...
   * synchronizer. Note that because timeouts and interrupts may occur at any time, a {@code true}
   * return does not guarantee that a future {@code signal} will awaken any threads.  This method is
  ...
  public class ConditionObject implements Condition, java.io.Serializable {
    ...
    /**
     * 1.与同步队列不同，条件队列头尾指针是firstWaiter跟lastWaiter 2.条件队列是在获取锁之后，也就是临界区进行操作，因此很多地方不用考虑并发
     */
    ...
    /**
     * 这里的判断逻辑是： 1.如果现在不是中断的，即正常被signal唤醒则返回0 2.如果节点由中断加入同步队列则返回THROW_IE，由signal加入同步队列则返回REINTERRUPT
     */
   ...
   * otherwise be done with atomic field updaters). unsafe魔法类，直接绕过虚拟机内存管理机制，修改内存
   */
